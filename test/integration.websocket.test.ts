...
beforeAll(async () => {
  ...
  serve(
    async (req) => {
      console.log('WebSocket server request:', req.url)
...
describe('WebSocket Browser Tests', () => {
  beforeAll(async () => {
  ...
  it('should test WebSocket transport and catch realtime-js 2.11.13 error', async () => {
    ...
    assertStringIncludes(logContent, 'Starting WebSocket test...')
    ...
    if (logContent.includes('Fetch called with URL:')) {
      console.log('WebSocket: HTTP requests were made (fetch calls detected)')
    } else {
      console.log('WebSocket: No HTTP requests detected')
    }

    if (logContent.includes('setTimeout called with delay:')) {
      console.log('WebSocket: setTimeout was used (setTimeout detected)')
    } else {
      console.log('WebSocket: No setTimeout detected')
    }

    if (logContent.includes('setInterval called with delay:')) {
      console.log('WebSocket: setInterval was used (setInterval detected)')
    } else {
      console.log('WebSocket: No setInterval detected')
    }
